...
"""
Initial Data Exploration for Dunnhumby The Complete Journey dataset
...
def explore_data():
    ...
    sns.barplot(x='total_sales', y='department', data=results.head(10))
    ...
    SELECT 
        ...
        ROUND(COUNT(DISTINCT t.basket_id)::numeric / COUNT(DISTINCT hd.household_key), 2) AS baskets_per_household
    ...
    SELECT 
        ...
        CASE 
            WHEN ch.targeted_households > 0 THEN 
                ROUND(COALESCE(cs.redeeming_households, 0)::numeric / ch.targeted_households * 100, 2)
            ELSE 0
